        ...
        try{
            ...
            for(user usr:User.selectAll()){
                users+="<tr><td>"+usr.getFirst_name()+"</td><td>"+usr.getLast_name()+"</td><td>"+usr.getEmail()+"</td>" +
                        "<td><form action='role' method='post'><select name='change_role'><option selected disabled>"+usr.getRole()+"</option><option>professor</option><option>student</option><option>admin</option></select>" +
